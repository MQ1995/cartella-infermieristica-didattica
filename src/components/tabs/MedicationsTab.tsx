          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-slate-600">
                ...
                <th className="px-2 py-2 text-left font-semibold w-32">Note</th>
                <th className="px-2 py-2 text-left font-semibold w-28">Firma</th>
            ...
            <tbody>
              {fields.map((field, index) => {
                ...
                return (
                  <tr key={field.id} className={`border-b border-slate-100 last:border-0 align-middle ${locked ? 'opacity-60' : 'hover:bg-slate-50'}`}>
                    ...
                    <td className="px-1 py-1">
                      <input {...register(`medications.${index}.notes`)} placeholder={locked ? '—' : 'Note'} disabled={locked} className={INPUT_CLS} />
                    </td>
                    <td className="px-1 py-1">
                      <input {...register(`medications.${index}.signature`)} placeholder={locked ? '—' : 'Firma'} disabled={locked} className={INPUT_CLS} />
                    </td>
